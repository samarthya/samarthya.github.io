...
def fetch_github_activity(username="samarthya", days=30):
    ...
    try:
        ...
        # Broad exception handling for network/API failures (timeouts, rate limits, etc.)
    ...
    for event in events:
        try:
            ...
            # Skip individual events that cause unexpected errors (malformed data, parsing issues)
            # Allows processing to continue for remaining events
            continue
...
if repos:
    ...
    for repo in repos:
        try:
            ...
            language_color = language.lower() if language else '#888'
